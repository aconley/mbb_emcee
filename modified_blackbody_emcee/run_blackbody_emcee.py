...
if results.fixT: lkl.fix_param(0)
if results.fixBeta: lkl.fix_param(1)
if results.fixLambda0: lkl.fix_param(2)
if results.noalpha or results.fixAlpha:
    lkl.fix_param(3)
if results.fixFnorm: lkl.fix_param(4)
        
# Lower limits
if not results.lowT is None: 
    lkl.set_lowlim(0,results.lowT)
if not results.lowBeta is None: 
    lkl.set_lowlim(1,results.lowBeta)
if (not results.opthin) and (not results.lowLambda0 is None): 
    lkl.set_lowlim(2,results.lowLambda0)
if (not results.noalpha) and (not results.lowAlpha is None): 
    lkl.set_lowlim(3,results.lowAlpha)
if not results.lowFnorm is None: 
    lkl.set_lowlim(4,results.lowFnorm)

# Upper Limits
if not results.upT is None: 
    lkl.set_uplim(0,results.upT)
if not results.upBeta is None : 
    lkl.set_uplim(1,results.upBeta)
if (not results.opthin) and (not results.upLambda0 is None): 
    lkl.set_uplim(2,results.upLambda0)
if (not results.noalpha) and (not results.upAlpha is None): 
    lkl.set_uplim(3,results.upAlpha)
if not results.upFnorm is None : 
    lkl.set_uplim(4,results.upFnorm)

# Priors
if not results.priorT is None:
    lkl.set_gaussian_prior(0, results.priorT[0], results.priorT[1])
if not results.priorBeta is None:
    lkl.set_gaussian_prior(1, results.priorBeta[0], results.priorBeta[1])
if (not results.opthin) and (not results.priorLambda0 is None):
    lkl.set_gaussian_prior(2, results.priorLambda0[0], 
                           results.priorLambda0[1])
if (not results.noalpha) and (not results.priorT is None):
    lkl.set_gaussian_prior(3, results.priorAlpha[0], results.priorAlpha[1])
if not results.priorFnorm is None:
    lkl.set_gaussian_prior(4, results.priorFnorm[0], results.priorFnorm[1])
...
# Create sampler
sampler = emcee.EnsembleSampler(nwalkers, npar, lkl,
